...
namespace Microservices.Adapters.EF.Oracle
{
    public class OracleAdapter : ISqlAdapter
    {
        ...
        public string Name { get => "OracleAdapter"; }
        public Type TargetType { get => typeof(ISqlAdapter); }
        public OracleAdapter()
        {
            string connstr = Config.Root["Oracle"];
            if (string.IsNullOrEmpty(connstr))
                throw new Exception("Config Not Found: [Oracle]");

            var tables = IoCFac.Instance.GetClassList<IEntity>()?.ToArray();

            this.context = new OracleContext(connstr, tables);
        }
